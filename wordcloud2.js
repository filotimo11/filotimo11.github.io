...
function checkDependencies() {
    if (typeof d3 === 'undefined') {
        ...
        errorDiv.textContent = 'D3.js library failed to load. Please check your internet connection and try again.';
        document.getElementById('llm-word-cloud').appendChild(errorDiv);
        return false;
    }
    if (typeof d3.layout.cloud === 'undefined') {
        const errorDiv = document.createElement('div');
        errorDiv.className = 'error-message';
        errorDiv.textContent = 'd3-cloud library failed to load. Please check your internet connection and try again.';
        document.getElementById('llm-word-cloud').appendChild(errorDiv);
        return false;
    }
    return true;
...
// 处理文本块以提取单词
function processChunk(chunk, wordFrequency, partialWord) {
        ...
        if (/[a-zA-Z]/.test(char)) {
            ...
            if (inWord) {
                ...
                if (!stopwords.has(lowerCaseWord)) {
                    wordFrequency.set(lowerCaseWord, (wordFrequency.get(lowerCaseWord) || 0) + 1);
                }
                inWord = false;
            }
        }
...
// 以较小的块加载并处理文件
async function processLargeFile(reader) {
    ...
    while (true) {
        ...
        partialWord = processChunk(chunk, wordFrequency, partialWord);
    }
...
// 使用 D3.js 生成词云
function generateWordCloud(wordFrequency) {
    ...
    const wordData = Array.from(wordFrequency.entries())
        .filter(([word]) => !stopwords.has(word) && /^[a-zA-Z]+$/.test(word)) // 过滤有效的英文单词
        .sort((a, b) => b[1] - a[1]) // 按频率排序
        .slice(0, 200) // 限制单词数量以提高性能
        .map(([word, count]) => ({ text: word, size: Math.sqrt(count) / 2.8 }));
...
function loadAndProcessFile() {
    return new Promise((resolve, reject) => {
        fetch('llm.txt')
            ...
            .then(reader => {
                return processLargeFile(reader);
            })
            .then(wordFrequency => {
                console.log('Word frequency data:', wordFrequency); // 调试：输出词频数据
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    loadAndProcessFile()
        .then(() => console.log('Word cloud generated successfully'))
        .catch(error => console.error('Word cloud generation failed:', error));
});